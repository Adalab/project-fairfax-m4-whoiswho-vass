...
class App extends React.Component {
  ...
  componentDidMount() {
    const newArr = employeeArr.map((item, index) => {
      return { ...item, id: `employeeId-${index}`, fullName: `${item.givenName} ${item.sn}` };
    });
    console.log(newArr);
    ...
    return (
      <div className="App">
        ...
        <ul className="employee__list">
          {/* {employeeArr
            .filter(item => item.givenName.includes(filterName) || item.sn.includes(filterName))
            .map(item =>
              <li className="employee__list--item">{item.givenName} {item.sn}</li>
            )
          } */}
          {nameArr
            .filter(item => item.fullName.includes(filterName))
            .map(item =>
              <li className="employee__list--item">{item.fullName}</li>
            )
          }
        </ul>
      </div>
